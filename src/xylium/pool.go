...
// the current HTTP request. The `Router` reference is typically set by the
// `Router.Handler` method shortly after the context is acquired.
...
func acquireCtx(originalCtx *fasthttp.RequestCtx) *Context {
	...
	// `c.reset()` should have already prepared other fields.
	return c
}
